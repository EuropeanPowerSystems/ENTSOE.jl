# Auto-generated from iec62325-451-6-transmissionnetwork_v4_2.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-6:transmissionnetworkdocument:4:2

module Transmissionnetwork_v4_2
...
Base.@kwdef mutable struct Point
    ...
    congestionCost_Price_amount::Union{Nothing, Float64} = nothing
    totalRedispatch_quantity_quantity::Union{Nothing, Float64} = nothing
end
...
Base.@kwdef mutable struct TimeSeries
    ...
    connecting_MarketParticipant_mRID::Union{Nothing, PartyID_String} = nothing
    connecting_MarketParticipant_marketRole_type::Union{Nothing, String} = nothing
    Period::Vector{Series_Period} = Series_Period[]
    Reason::Vector{Reason} = Reason[]
end

Base.@kwdef mutable struct TransmissionNetwork_MarketDocument
    mRID::String = ""
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "congestionCost_Price.amount"
            out.congestionCost_Price_amount = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "totalRedispatch_quantity.quantity"
            out.totalRedispatch_quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "connecting_MarketParticipant.mRID"
            out.connecting_MarketParticipant_mRID = _parse(PartyID_String, c)
        elseif nm == "connecting_MarketParticipant.marketRole.type"
            out.connecting_MarketParticipant_marketRole_type = String(strip(nodecontent(c)))
        elseif nm == "Period"
...
function _parse(::Type{TransmissionNetwork_MarketDocument}, n::EzXML.Node)
    out = TransmissionNetwork_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::TransmissionNetwork_MarketDocument
    return _parse(TransmissionNetwork_MarketDocument, root(parsexml(xml)))
end

end  # module Transmissionnetwork_v4_2
