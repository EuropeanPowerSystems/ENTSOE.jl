# Auto-generated from iec62325-451-n-rgcesettlement_v1_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-n:rgcesettlementdocument:1:0

module Rgcesettlement_v1_0
...
Base.@kwdef mutable struct Point
    ...
    secondaryQuantity::Float64 = 0.0
    aceTariffType_type::Union{Nothing, String} = nothing
end
...
Base.@kwdef mutable struct DomainAccount_AceTariffType
    type_::String = ""
    opening_Point_quantity::Float64 = 0.0
    closing_Point_quantity::Float64 = 0.0
    compensation_Point_quantity::Union{Nothing, Float64} = nothing
    measure_Unit_name::String = ""
end

Base.@kwdef mutable struct RGCEsettlement_MarketDocument
    mRID::String = ""
    ...
    recording_Period_timeInterval::ESMP_DateTimeInterval = ESMP_DateTimeInterval()
    compensation_Period_timeInterval::Union{Nothing, ESMP_DateTimeInterval} = nothing
    domain_mRID::AreaID_String = AreaID_String()
    TimeSeries::Vector{TimeSeries} = TimeSeries[]
    DomainAccount_AceTariffType::Vector{DomainAccount_AceTariffType} = DomainAccount_AceTariffType[]
end
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "aceTariffType.type"
            out.aceTariffType_type = String(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{DomainAccount_AceTariffType}, n::EzXML.Node)
    out = DomainAccount_AceTariffType()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "type"
            out.type_ = String(strip(nodecontent(c)))
        elseif nm == "opening_Point.quantity"
            out.opening_Point_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "closing_Point.quantity"
            out.closing_Point_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "compensation_Point.quantity"
            out.compensation_Point_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "measure_Unit.name"
            out.measure_Unit_name = String(strip(nodecontent(c)))
        end
    end
...
function _parse(::Type{RGCEsettlement_MarketDocument}, n::EzXML.Node)
    out = RGCEsettlement_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
        ...
        elseif nm == "recording_Period.timeInterval"
            out.recording_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
        elseif nm == "compensation_Period.timeInterval"
            out.compensation_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
        elseif nm == "domain.mRID"
            out.domain_mRID = _parse(AreaID_String, c)
        elseif nm == "TimeSeries"
            push!(out.TimeSeries, _parse(TimeSeries, c))
        elseif nm == "DomainAccount_AceTariffType"
            push!(out.DomainAccount_AceTariffType, _parse(DomainAccount_AceTariffType, c))
        end
    end
    return out
end
...
function parse_document(xml::AbstractString)::RGCEsettlement_MarketDocument
    return _parse(RGCEsettlement_MarketDocument, root(parsexml(xml)))
end

end  # module Rgcesettlement_v1_0
