# Auto-generated from iec62325-451-n-weatherprognosisdocument_v1_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-n:energyprognosisdocument:1:0

module Weatherprognosisdocument_v1_0
...
Base.@kwdef mutable struct EnergyPrognosis_MarketDocument
    mRID::String = ""
    ...
    Area_TimeSeries::Vector{TimeSeries} = TimeSeries[]
end
...
function _parse(::Type{EnergyPrognosis_MarketDocument}, n::EzXML.Node)
    out = EnergyPrognosis_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
        ...
        elseif nm == "Area_TimeSeries"
            push!(out.Area_TimeSeries, _parse(TimeSeries, c))
        end
    end
    return out
end
...
function parse_document(xml::AbstractString)::EnergyPrognosis_MarketDocument
    return _parse(EnergyPrognosis_MarketDocument, root(parsexml(xml)))
end

end  # module Weatherprognosisdocument_v1_0
