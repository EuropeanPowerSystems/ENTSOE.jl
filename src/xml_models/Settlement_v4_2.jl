# Auto-generated from iec62325-451-4-settlement_v4_2.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-4:energyaccountdocument:4:2

module Settlement_v4_2
...
Base.@kwdef mutable struct Point
    position::Int64 = 0
    in_Quantity_quantity::Float64 = 0.0
    in_Quantity_quality::Union{Nothing, String} = nothing
    out_Quantity_quantity::Float64 = 0.0
    out_Quantity_quality::Union{Nothing, String} = nothing
...
Base.@kwdef mutable struct Party_MarketParticipant
    ...
    party_MarketRole_type::Union{Nothing, String} = nothing
end
...
Base.@kwdef mutable struct TimeSeries
    ...
    MarketParticipant::Vector{Party_MarketParticipant} = Party_MarketParticipant[]
    ...
    accountingPoint_settlementMethod::Union{Nothing, String} = nothing
    Period::Vector{Series_Period} = Series_Period[]
    Reason::Vector{Reason} = Reason[]
end

Base.@kwdef mutable struct EnergyAccount_MarketDocument
    mRID::String = ""
    revisionNumber::String = ""
    type_::String = ""
    docStatus::Action_Status = Action_Status()
    process_processType::String = ""
    process_classificationType::String = ""
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "in_Quantity.quantity"
            out.in_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "in_Quantity.quality"
            out.in_Quantity_quality = String(strip(nodecontent(c)))
        elseif nm == "out_Quantity.quantity"
            out.out_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "out_Quantity.quality"
            out.out_Quantity_quality = String(strip(nodecontent(c)))
...
function _parse(::Type{Party_MarketParticipant}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "party_MarketRole.type"
            out.party_MarketRole_type = String(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "MarketParticipant"
            push!(out.MarketParticipant, _parse(Party_MarketParticipant, c))
        ...
        elseif nm == "accountingPoint.settlementMethod"
            out.accountingPoint_settlementMethod = String(strip(nodecontent(c)))
        elseif nm == "Period"
...
function _parse(::Type{EnergyAccount_MarketDocument}, n::EzXML.Node)
    out = EnergyAccount_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::EnergyAccount_MarketDocument
    return _parse(EnergyAccount_MarketDocument, root(parsexml(xml)))
end

end  # module Settlement_v4_2
