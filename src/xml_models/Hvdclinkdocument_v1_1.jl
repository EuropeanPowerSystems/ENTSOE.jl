# Auto-generated from iec62325-451-8-hvdclinkdocument_v1_1.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-8:hvdclinkdocument:1:1

module Hvdclinkdocument_v1_1
...
Base.@kwdef mutable struct Point
    ...
    optimum_Quantity_quantity::Union{Nothing, Float64} = nothing
end
...
Base.@kwdef mutable struct TimeSeries
    ...
    hVDCMode_AttributeInstanceComponent_attribute::Union{Nothing, String} = nothing
    ...
    minimumExchange_Quantity_quantity::Union{Nothing, Float64} = nothing
    maximumExchange_Quantity_quantity::Union{Nothing, Float64} = nothing
    start_DateAndOrTime_dateTime::Union{Nothing, DateTime} = nothing
    end_DateAndOrTime_dateTime::Union{Nothing, DateTime} = nothing
    Period::Vector{Series_Period} = Series_Period[]
    Reason::Vector{Reason} = Reason[]
end

Base.@kwdef mutable struct HVDCLink_MarketDocument
    mRID::String = ""
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "optimum_Quantity.quantity"
            out.optimum_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "hVDCMode_AttributeInstanceComponent.attribute"
            out.hVDCMode_AttributeInstanceComponent_attribute = String(strip(nodecontent(c)))
        ...
        elseif nm == "minimumExchange_Quantity.quantity"
            out.minimumExchange_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "maximumExchange_Quantity.quantity"
            out.maximumExchange_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "start_DateAndOrTime.dateTime"
            out.start_DateAndOrTime_dateTime = (x -> DateTime(replace(String(x), "Z" => "")))(strip(nodecontent(c)))
        elseif nm == "end_DateAndOrTime.dateTime"
            out.end_DateAndOrTime_dateTime = (x -> DateTime(replace(String(x), "Z" => "")))(strip(nodecontent(c)))
        elseif nm == "Period"
...
function _parse(::Type{HVDCLink_MarketDocument}, n::EzXML.Node)
    out = HVDCLink_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::HVDCLink_MarketDocument
    return _parse(HVDCLink_MarketDocument, root(parsexml(xml)))
end

end  # module Hvdclinkdocument_v1_1
