# Auto-generated from iec62325-451-7-resourcescheduleconfirmation_v6_2.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-7:resourcescheduleconfirmationdocument:6:2

module Resourcescheduleconfirmation_v6_2
...
Base.@kwdef mutable struct UnavailableReserve_TimeSeries
    ...
    connecting_Domain_mRID::AreaID_String = AreaID_String()
    registeredResource_mRID::Union{Nothing, ResourceID_String} = nothing
    substitute_RegisteredResource_mRID::Union{Nothing, ResourceID_String} = nothing
    resourceProvider_MarketParticipant_mRID::PartyID_String = PartyID_String()
    substituteResourceProvider_MarketParticipant_mRID::Union{Nothing, PartyID_String} = nothing
    substituteResourceProvider_MarketParticipant_marketRole_type::Union{Nothing, String} = nothing
...
Base.@kwdef mutable struct Original_MarketDocument
    ...
    UnavailableReserve_TimeSeries::Vector{UnavailableReserve_TimeSeries} = UnavailableReserve_TimeSeries[]
end

Base.@kwdef mutable struct ResourceScheduleConfirmation_MarketDocument
    mRID::String = ""
    type_::String = ""
    ...
    Original_MarketDocument::Original_MarketDocument = Original_MarketDocument()
    Reason::Vector{Reason} = Reason[]
end
...
function _parse(::Type{UnavailableReserve_TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "registeredResource.mRID"
            out.registeredResource_mRID = _parse(ResourceID_String, c)
        elseif nm == "substitute_RegisteredResource.mRID"
            out.substitute_RegisteredResource_mRID = _parse(ResourceID_String, c)
        elseif nm == "resourceProvider_MarketParticipant.mRID"
            out.resourceProvider_MarketParticipant_mRID = _parse(PartyID_String, c)
        ...
        elseif nm == "substituteResourceProvider_MarketParticipant.marketRole.type"
            out.substituteResourceProvider_MarketParticipant_marketRole_type = String(strip(nodecontent(c)))
        elseif nm == "acquiring_Domain.mRID"
            out.acquiring_Domain_mRID = _parse(AreaID_String, c)
        elseif nm == "marketAgreement.type"
            out.marketAgreement_type = String(strip(nodecontent(c)))
...
function _parse(::Type{Original_MarketDocument}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "UnavailableReserve_TimeSeries"
            push!(out.UnavailableReserve_TimeSeries, _parse(UnavailableReserve_TimeSeries, c))
        end
    end
    return out
end

function _parse(::Type{ResourceScheduleConfirmation_MarketDocument}, n::EzXML.Node)
    out = ResourceScheduleConfirmation_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
        ...
        elseif nm == "Original_MarketDocument"
            out.Original_MarketDocument = _parse(Original_MarketDocument, c)
        elseif nm == "Reason"
            push!(out.Reason, _parse(Reason, c))
        end
    end
...
function parse_document(xml::AbstractString)::ResourceScheduleConfirmation_MarketDocument
    return _parse(ResourceScheduleConfirmation_MarketDocument, root(parsexml(xml)))
end

end  # module Resourcescheduleconfirmation_v6_2
