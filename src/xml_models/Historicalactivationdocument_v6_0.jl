# Auto-generated from iec62325-451-7-historicalactivationdocument_v6_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-7:historicalactivationdocument:6:0

module Historicalactivationdocument_v6_0
...
Base.@kwdef mutable struct TimeSeries
    activation_Original_MarketDocument_mRID::Union{Nothing, String} = nothing
    activation_Original_MarketDocument_revisionNumber::Union{Nothing, String} = nothing
    activation_Original_MarketDocument_activation_Process_processType::Union{Nothing, String} = nothing
    activation_Original_MarketDocument_requesting_MarketParticipant_mRID::Union{Nothing, PartyID_String} = nothing
    activation_Original_MarketDocument_activating_MarketParticipant_mRID::Union{Nothing, PartyID_String} = nothing
    activation_Original_MarketDocument_activationTime_Period_timeInterval::Union{Nothing, ESMP_DateTimeInterval} = nothing
    mRID::String = ""
...
Base.@kwdef mutable struct HistoricalActivation_MarketDocument
    mRID::String = ""
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        local nm = nodename(c)
        if nm == "activation_Original_MarketDocument.mRID"
            out.activation_Original_MarketDocument_mRID = String(strip(nodecontent(c)))
        elseif nm == "activation_Original_MarketDocument.revisionNumber"
            out.activation_Original_MarketDocument_revisionNumber = String(strip(nodecontent(c)))
        elseif nm == "activation_Original_MarketDocument.activation_Process.processType"
            out.activation_Original_MarketDocument_activation_Process_processType = String(strip(nodecontent(c)))
        elseif nm == "activation_Original_MarketDocument.requesting_MarketParticipant.mRID"
            out.activation_Original_MarketDocument_requesting_MarketParticipant_mRID = _parse(PartyID_String, c)
        elseif nm == "activation_Original_MarketDocument.activating_MarketParticipant.mRID"
            out.activation_Original_MarketDocument_activating_MarketParticipant_mRID = _parse(PartyID_String, c)
        elseif nm == "activation_Original_MarketDocument.activationTime_Period.timeInterval"
            out.activation_Original_MarketDocument_activationTime_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
        elseif nm == "mRID"
...
function _parse(::Type{HistoricalActivation_MarketDocument}, n::EzXML.Node)
    out = HistoricalActivation_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::HistoricalActivation_MarketDocument
    return _parse(HistoricalActivation_MarketDocument, root(parsexml(xml)))
end

end  # module Historicalactivationdocument_v6_0
