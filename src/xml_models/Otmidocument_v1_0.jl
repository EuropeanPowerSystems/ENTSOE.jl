# Auto-generated from iec62325-451-n-otmidocument_v1_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-n:otmidocument:1:0

module Otmidocument_v1_0
...
Base.@kwdef mutable struct OtherTransparencyMarketInformation_MarketDocument
    mRID::String = ""
    ...
    publication_DateAndOrTime_dateTime::DateTime = DateTime(1970)
    start_DateAndOrTime_dateTime::DateTime = DateTime(1970)
    end_DateAndOrTime_dateTime::Union{Nothing, DateTime} = nothing
    reason_code::String = ""
    reason_text::Union{Nothing, String} = nothing
end
...
function _parse(::Type{OtherTransparencyMarketInformation_MarketDocument}, n::EzXML.Node)
    out = OtherTransparencyMarketInformation_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
        ...
        elseif nm == "publication_DateAndOrTime.dateTime"
            out.publication_DateAndOrTime_dateTime = (x -> DateTime(replace(String(x), "Z" => "")))(strip(nodecontent(c)))
        ...
        elseif nm == "reason.code"
            out.reason_code = String(strip(nodecontent(c)))
        elseif nm == "reason.text"
            out.reason_text = String(strip(nodecontent(c)))
        end
    end
    return out
end
...
function parse_document(xml::AbstractString)::OtherTransparencyMarketInformation_MarketDocument
    return _parse(OtherTransparencyMarketInformation_MarketDocument, root(parsexml(xml)))
end

end  # module Otmidocument_v1_0
