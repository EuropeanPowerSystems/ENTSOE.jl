# Auto-generated from iec62325-451-7-redispatchdocument_v6_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-7:redispatchdocument:6:0

module Redispatchdocument_v6_0
...
Base.@kwdef mutable struct Point
    ...
    price_category::Union{Nothing, String} = nothing
    price_amount::Union{Nothing, Float64} = nothing
    price_direction::Union{Nothing, String} = nothing
end
...
Base.@kwdef mutable struct TimeSeries
    mRID::String = ""
    dispatcher_MarketParticipant_name::String = ""
    businessType::String = ""
    impacted_Domain_mRID::AreaID_String = AreaID_String()
    ...
    total_Price_amount::Union{Nothing, Float64} = nothing
    total_Price_direction::Union{Nothing, String} = nothing
    Period::Vector{Series_Period} = Series_Period[]
end

Base.@kwdef mutable struct Redispatch_MarketDocument
    mRID::String = ""
    ...
    redispatch_Time_Period_timeInterval::ESMP_DateTimeInterval = ESMP_DateTimeInterval()
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "price.category"
            out.price_category = String(strip(nodecontent(c)))
        elseif nm == "price.amount"
            out.price_amount = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "price.direction"
            out.price_direction = String(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "dispatcher_MarketParticipant.name"
            out.dispatcher_MarketParticipant_name = String(strip(nodecontent(c)))
        elseif nm == "businessType"
            out.businessType = String(strip(nodecontent(c)))
        elseif nm == "impacted_Domain.mRID"
            out.impacted_Domain_mRID = _parse(AreaID_String, c)
        ...
        elseif nm == "total_Price.amount"
            out.total_Price_amount = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "total_Price.direction"
            out.total_Price_direction = String(strip(nodecontent(c)))
        elseif nm == "Period"
...
function _parse(::Type{Redispatch_MarketDocument}, n::EzXML.Node)
    out = Redispatch_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
        ...
        elseif nm == "redispatch_Time_Period.timeInterval"
            out.redispatch_Time_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
...
function parse_document(xml::AbstractString)::Redispatch_MarketDocument
    return _parse(Redispatch_MarketDocument, root(parsexml(xml)))
end

end  # module Redispatchdocument_v6_0
