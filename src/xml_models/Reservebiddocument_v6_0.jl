# Auto-generated from iec62325-451-7-reservebiddocument_v6_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-7:reservebiddocument:6:0

module Reservebiddocument_v6_0
...
Base.@kwdef mutable struct MBA_Domain
...
Base.@kwdef mutable struct BidTimeSeries
    ...
    minimumActivationQuantity::Union{Nothing, Float64} = nothing
    ...
    AvailableMBA_Domain::Vector{MBA_Domain} = MBA_Domain[]
end
...
function _parse(::Type{MBA_Domain}, n::EzXML.Node)
    out = MBA_Domain()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
...
function _parse(::Type{BidTimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "minimumActivationQuantity"
            out.minimumActivationQuantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        ...
        elseif nm == "AvailableMBA_Domain"
            push!(out.AvailableMBA_Domain, _parse(MBA_Domain, c))
        end
    end
    return out
end
...
end  # module Reservebiddocument_v6_0
