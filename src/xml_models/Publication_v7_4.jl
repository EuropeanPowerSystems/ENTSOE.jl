# Auto-generated from iec62325-451-3-publication_v7_4.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-3:publicationdocument:7:4

module Publication_v7_4
...
Base.@kwdef mutable struct Winners_MarketParticipant
...
Base.@kwdef mutable struct TimeSeries
    ...
    auction_type::Union{Nothing, String} = nothing
    ...
    contract_MarketAgreement_type::Union{Nothing, String} = nothing
    ...
    classificationSequence_AttributeInstanceComponent_position::Union{Nothing, Int64} = nothing
    participantNumber_AttributeInstanceComponent_position::Union{Nothing, Int64} = nothing
    winnerParticipantNumber_AttributeInstanceComponent_position::Union{Nothing, Int64} = nothing
    curveType::Union{Nothing, String} = nothing
    update_DateAndOrTime_dateTime::Union{Nothing, DateTime} = nothing
    connectingLine_RegisteredResource_mRID::Union{Nothing, ResourceID_String} = nothing
    Period::Vector{Series_Period} = Series_Period[]
    Reason::Vector{Reason} = Reason[]
    Winners_MarketParticipant::Vector{Winners_MarketParticipant} = Winners_MarketParticipant[]
end

Base.@kwdef mutable struct Publication_MarketDocument
    mRID::String = ""
...
function _parse(::Type{Winners_MarketParticipant}, n::EzXML.Node)
    out = Winners_MarketParticipant()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "classificationSequence_AttributeInstanceComponent.position"
            out.classificationSequence_AttributeInstanceComponent_position = (x -> parse(Int64, x))(strip(nodecontent(c)))
        elseif nm == "participantNumber_AttributeInstanceComponent.position"
            out.participantNumber_AttributeInstanceComponent_position = (x -> parse(Int64, x))(strip(nodecontent(c)))
        elseif nm == "winnerParticipantNumber_AttributeInstanceComponent.position"
            out.winnerParticipantNumber_AttributeInstanceComponent_position = (x -> parse(Int64, x))(strip(nodecontent(c)))
        elseif nm == "curveType"
            out.curveType = String(strip(nodecontent(c)))
        elseif nm == "update_DateAndOrTime.dateTime"
            out.update_DateAndOrTime_dateTime = (x -> DateTime(replace(String(x), "Z" => "")))(strip(nodecontent(c)))
        ...
        elseif nm == "Reason"
            push!(out.Reason, _parse(Reason, c))
        elseif nm == "Winners_MarketParticipant"
            push!(out.Winners_MarketParticipant, _parse(Winners_MarketParticipant, c))
        end
    end
    return out
end

function _parse(::Type{Publication_MarketDocument}, n::EzXML.Node)
...
function parse_document(xml::AbstractString)::Publication_MarketDocument
    return _parse(Publication_MarketDocument, root(parsexml(xml)))
end

end  # module Publication_v7_4
