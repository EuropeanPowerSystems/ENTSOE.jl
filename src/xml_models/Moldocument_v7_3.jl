# Auto-generated from iec62325-451-7-moldocument_v7_3.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-7:moldocument:7:3

module Moldocument_v7_3
...
Base.@kwdef mutable struct Point
    ...
    activated_Quantity_quantity::Union{Nothing, Float64} = nothing
end
...
Base.@kwdef mutable struct BidTimeSeries
    ...
    auction_mRID::String = ""
    auction_paymentTerms::Union{Nothing, String} = nothing
    businessType::String = ""
    bid_Period_timeInterval::ESMP_DateTimeInterval = ESMP_DateTimeInterval()
    ...
    energyPrice_Measurement_Unit_name::Union{Nothing, String} = nothing
    direction::String = ""
...
Base.@kwdef mutable struct MeritOrderList_MarketDocument
    mRID::String = ""
    ...
    relatedReserveBid_MarketDocument_mRID::Union{Nothing, String} = nothing
    relatedReserveBid_MarketDocument_revisionNumber::Union{Nothing, String} = nothing
    TimeSeries::Vector{BidTimeSeries} = BidTimeSeries[]
    Reason::Vector{Reason} = Reason[]
end
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "activated_Quantity.quantity"
            out.activated_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{BidTimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "bid_Period.timeInterval"
            out.bid_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
        ...
        elseif nm == "energyPrice_Measurement_Unit.name"
            out.energyPrice_Measurement_Unit_name = String(strip(nodecontent(c)))
        elseif nm == "direction"
...
function _parse(::Type{MeritOrderList_MarketDocument}, n::EzXML.Node)
    out = MeritOrderList_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
        ...
        elseif nm == "relatedReserveBid_MarketDocument.mRID"
            out.relatedReserveBid_MarketDocument_mRID = String(strip(nodecontent(c)))
        elseif nm == "relatedReserveBid_MarketDocument.revisionNumber"
            out.relatedReserveBid_MarketDocument_revisionNumber = String(strip(nodecontent(c)))
        elseif nm == "TimeSeries"
            push!(out.TimeSeries, _parse(BidTimeSeries, c))
        elseif nm == "Reason"
            push!(out.Reason, _parse(Reason, c))
        end
    end
...
function parse_document(xml::AbstractString)::MeritOrderList_MarketDocument
    return _parse(MeritOrderList_MarketDocument, root(parsexml(xml)))
end

end  # module Moldocument_v7_3
