# Auto-generated from iec62325-451-6-generationload_v3_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-6:generationloaddocument:3:0

module Generationload_v3_0
...
Base.@kwdef mutable struct MktGeneratingUnit
    ...
    nominalP::Union{Nothing, ESMP_ActivePower} = nothing
end
...
Base.@kwdef mutable struct MktPSRType
    psrType::String = ""
    voltage_PowerSystemResources_highVoltageLimit::Union{Nothing, ESMP_Voltage} = nothing
    PowerSystemResources::Vector{MktGeneratingUnit} = MktGeneratingUnit[]
end
...
Base.@kwdef mutable struct TimeSeries
    ...
    inBiddingZone_Domain_mRID::Union{Nothing, AreaID_String} = nothing
    outBiddingZone_Domain_mRID::Union{Nothing, AreaID_String} = nothing
    registeredResource_mRID::Union{Nothing, ResourceID_String} = nothing
    registeredResource_name::Union{Nothing, String} = nothing
    quantity_Measure_Unit_name::String = ""
    curveType::String = ""
    cancelledTS::Union{Nothing, String} = nothing
    MktPSRType::Union{Nothing, MktPSRType} = nothing
    Period::Vector{Series_Period} = Series_Period[]
end

Base.@kwdef mutable struct GL_MarketDocument
    mRID::String = ""
...
function _parse(::Type{MktPSRType}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "voltage_PowerSystemResources.highVoltageLimit"
            out.voltage_PowerSystemResources_highVoltageLimit = _parse(ESMP_Voltage, c)
        elseif nm == "PowerSystemResources"
            push!(out.PowerSystemResources, _parse(MktGeneratingUnit, c))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "inBiddingZone_Domain.mRID"
            out.inBiddingZone_Domain_mRID = _parse(AreaID_String, c)
        elseif nm == "outBiddingZone_Domain.mRID"
            out.outBiddingZone_Domain_mRID = _parse(AreaID_String, c)
...
function _parse(::Type{GL_MarketDocument}, n::EzXML.Node)
    out = GL_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::GL_MarketDocument
    return _parse(GL_MarketDocument, root(parsexml(xml)))
end

end  # module Generationload_v3_0
