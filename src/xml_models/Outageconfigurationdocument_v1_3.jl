# Auto-generated from iec62325-451-n-outageconfigurationdocument_v1_3.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-n:outageconfigurationdocument:1:3

module Outageconfigurationdocument_v1_3
...
Base.@kwdef mutable struct RegisteredResource
    ...
    pSRType_powerSystemResources_highVoltageLimit::ESMP_Voltage = ESMP_Voltage()
    pSRType_powerSystemResources_lowVoltageLimit::Union{Nothing, ESMP_Voltage} = nothing
    interesting_MarketObjectStatus_status::Union{Nothing, String} = nothing
    relevant_MarketObjectStatus_status::Union{Nothing, String} = nothing
    Associated_Domain::Vector{Domain} = Domain[]
end
...
Base.@kwdef mutable struct TimeSeries
    ...
    Relevant_MarketParticipant::Vector{Other_MarketParticipant} = Other_MarketParticipant[]
    Specific_RegisteredResource::Vector{Specific_RegisteredResource} = Specific_RegisteredResource[]
end

Base.@kwdef mutable struct OutageConfiguration_MarketDocument
    mRID::String = ""
    type_::String = ""
...
function _parse(::Type{RegisteredResource}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "interesting_MarketObjectStatus.status"
            out.interesting_MarketObjectStatus_status = String(strip(nodecontent(c)))
        elseif nm == "relevant_MarketObjectStatus.status"
            out.relevant_MarketObjectStatus_status = String(strip(nodecontent(c)))
        elseif nm == "Associated_Domain"
            push!(out.Associated_Domain, _parse(Domain, c))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "Relevant_MarketParticipant"
            push!(out.Relevant_MarketParticipant, _parse(Other_MarketParticipant, c))
...
function _parse(::Type{OutageConfiguration_MarketDocument}, n::EzXML.Node)
    out = OutageConfiguration_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
...
function parse_document(xml::AbstractString)::OutageConfiguration_MarketDocument
    return _parse(OutageConfiguration_MarketDocument, root(parsexml(xml)))
end

end  # module Outageconfigurationdocument_v1_3
