# Auto-generated from iec62325-451-7-moldocument_v6_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-7:moldocument:6:0

module Moldocument_v6_0
...
Base.@kwdef mutable struct MOL_TimeSeries
    ...
    quantityMeasurement_Unit_name::String = ""
    currency_Unit_name::Union{Nothing, String} = nothing
    priceMeasurement_Unit_name::Union{Nothing, String} = nothing
    energyPriceMeasurement_Unit_name::Union{Nothing, String} = nothing
...
Base.@kwdef mutable struct MeritOrderList_MarketDocument
    ...
    valid_Time_Period_timeInterval::ESMP_DateTimeInterval = ESMP_DateTimeInterval()
    domain_mRID::Union{Nothing, AreaID_String} = nothing
    MOL_TimeSeries::Vector{MOL_TimeSeries} = MOL_TimeSeries[]
end
...
function _parse(::Type{MOL_TimeSeries}, n::EzXML.Node)
    out = MOL_TimeSeries()
    for c in elements(n)
        local nm = nodename(c)
        ...
        elseif nm == "quantityMeasurement_Unit.name"
            out.quantityMeasurement_Unit_name = String(strip(nodecontent(c)))
        elseif nm == "currency_Unit.name"
            out.currency_Unit_name = String(strip(nodecontent(c)))
        elseif nm == "priceMeasurement_Unit.name"
            out.priceMeasurement_Unit_name = String(strip(nodecontent(c)))
        elseif nm == "energyPriceMeasurement_Unit.name"
            out.energyPriceMeasurement_Unit_name = String(strip(nodecontent(c)))
...
function _parse(::Type{MeritOrderList_MarketDocument}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "valid_Time_Period.timeInterval"
            out.valid_Time_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
        elseif nm == "domain.mRID"
            out.domain_mRID = _parse(AreaID_String, c)
        elseif nm == "MOL_TimeSeries"
            push!(out.MOL_TimeSeries, _parse(MOL_TimeSeries, c))
        end
    end
    return out
end
