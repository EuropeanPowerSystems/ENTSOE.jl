# Auto-generated from iec62325-451-n-smtaprognosis_v_1_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-n:smtaprognosisdocument:1:0

module Smtaprognosis_v_1_0
...
Base.@kwdef mutable struct Percentile_Quantity
    type_::String = ""
...
Base.@kwdef mutable struct Point
    position::Int64 = 0
    quantity::Float64 = 0.0
    generationOutageProbability_Quantity_quantity::Union{Nothing, Float64} = nothing
    Percentile_Quantity::Vector{Percentile_Quantity} = Percentile_Quantity[]
end
...
Base.@kwdef mutable struct ShortMediumTermAdequacyPrognosis_MarketDocument
    mRID::String = ""
    revisionNumber::String = ""
    type_::String = ""
    processType_Process_processType::String = ""
...
function _parse(::Type{Percentile_Quantity}, n::EzXML.Node)
    out = Percentile_Quantity()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "type"
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "quantity"
            out.quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "generationOutageProbability_Quantity.quantity"
            out.generationOutageProbability_Quantity_quantity = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "Percentile_Quantity"
            push!(out.Percentile_Quantity, _parse(Percentile_Quantity, c))
        end
    end
    return out
end
...
function _parse(::Type{ShortMediumTermAdequacyPrognosis_MarketDocument}, n::EzXML.Node)
    out = ShortMediumTermAdequacyPrognosis_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
        ...
        elseif nm == "type"
            out.type_ = String(strip(nodecontent(c)))
        elseif nm == "processType_Process.processType"
            out.processType_Process_processType = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::ShortMediumTermAdequacyPrognosis_MarketDocument
    return _parse(ShortMediumTermAdequacyPrognosis_MarketDocument, root(parsexml(xml)))
end

end  # module Smtaprognosis_v_1_0
