# Auto-generated from iec62325-451-n-rasettlement_v1_2.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-n:rasettlementdocument:1:2

module Rasettlement_v1_2
...
Base.@kwdef mutable struct Point
    position::Int64 = 0
    credit_Price_amount::Float64 = 0.0
    debit_Price_amount::Float64 = 0.0
end
...
Base.@kwdef mutable struct TimeSeries
    ...
    marketProduct_marketProductType::Union{Nothing, String} = nothing
    Period::Vector{Series_Period} = Series_Period[]
end

Base.@kwdef mutable struct RASettlement_MarketDocument
    mRID::String = ""
...
function _parse(::Type{Point}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "credit_Price.amount"
            out.credit_Price_amount = (x -> parse(Float64, x))(strip(nodecontent(c)))
        elseif nm == "debit_Price.amount"
            out.debit_Price_amount = (x -> parse(Float64, x))(strip(nodecontent(c)))
        end
    end
    return out
end
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "marketProduct.marketProductType"
            out.marketProduct_marketProductType = String(strip(nodecontent(c)))
        elseif nm == "Period"
...
function _parse(::Type{RASettlement_MarketDocument}, n::EzXML.Node)
    out = RASettlement_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
...
function parse_document(xml::AbstractString)::RASettlement_MarketDocument
    return _parse(RASettlement_MarketDocument, root(parsexml(xml)))
end

end  # module Rasettlement_v1_2
