# Auto-generated from iec62325-451-2-schedule_v5_0.xsd — DO NOT EDIT
# Re-run `gen/regenerate_xml_models.jl` to regenerate.
# Source schema namespace: urn:iec62325.351:tc57wg16:451-2:scheduledocument:5:0

module Schedule_v5_0
...
Base.@kwdef mutable struct TimeSeries
    ...
    Reason::Union{Nothing, Reason} = nothing
end

Base.@kwdef mutable struct Schedule_MarketDocument
    mRID::String = ""
    ...
    matching_Time_Period_timeInterval::Union{Nothing, ESMP_DateTimeInterval} = nothing
...
function _parse(::Type{TimeSeries}, n::EzXML.Node)
    ...
    for c in elements(n)
        ...
        elseif nm == "Reason"
            out.Reason = _parse(Reason, c)
        end
    end
    return out
end

function _parse(::Type{Schedule_MarketDocument}, n::EzXML.Node)
    out = Schedule_MarketDocument()
    for c in elements(n)
        local nm = nodename(c)
        if nm == "mRID"
            out.mRID = String(strip(nodecontent(c)))
        ...
        elseif nm == "matching_Time_Period.timeInterval"
            out.matching_Time_Period_timeInterval = _parse(ESMP_DateTimeInterval, c)
...
function parse_document(xml::AbstractString)::Schedule_MarketDocument
    return _parse(Schedule_MarketDocument, root(parsexml(xml)))
end

end  # module Schedule_v5_0
