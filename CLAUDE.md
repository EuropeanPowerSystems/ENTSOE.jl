...
src/
  conveniences/   ← hand-written ENTSO-E layer (untouched by codegen)
    ...
    eic.jl        ←   EIC named tuple + EIC_REGISTRY + NEIGHBOURS +
                      lookup_eic/is_known_eic/eics_of_type/validate_eic
  ...
  client/         ← hand-written template overlay (also untouched by codegen)
    ...
    auth.jl       ←   Auth/NoAuth + build_pre_request_hook (ENTSO-E auth is a
                      query parameter — header strategies don't apply)
...
gen/
  regenerate.jl   ← rerun codegen against spec/openapi.json; also refreshes
                    docs/src/api/<Tag>.md + the base-path section of
                    docs/src/generated_reference.md (via emit_api_pages.jl)
spec/
...
- Need to know what `gen/regenerate.jl` will overwrite? `src/api/` plus the spec-derived docs pages (`docs/src/api/<Tag>.md`, the base-path section of `docs/src/generated_reference.md`) — never `src/ENTSOE.jl`, `src/client/`, `src/conveniences/`, `test/`, or hand-written docs.
